...
## DEC-029 — `parallel-ready` queda canonizado como elegibilidad controlada con `git worktree + una rama por BL`
- **Fecha:** 2026-03-10
- **Contexto:** El repo ya había documentado aprendizajes parciales sobre paralelización mutante en `docs/project/KNOWLEDGE_BASE.md`, pero faltaba una fuente de verdad operativa y contractual que fijara cuándo una sesión puede tratarse como `parallel-ready`, qué surfaces siguen serializadas, cómo se define el `write_set`, qué rol conserva el padre neutral y cómo se aborta sin contaminar `main`. Esa ausencia dejaba `BL-072` abierta y `BL-073` bloqueada solo por falta de canonización documental.
- **Decisión:**
  1. `parallel-ready` se canoniza como **elegibilidad operativa controlada para una sesión concreta**, no como permiso general para mutar el repo en paralelo ni como autorización para escribir concurrentemente sobre el mismo árbol principal.
  2. El modelo oficial de mutación paralela pasa a ser `git worktree + una rama por BL`, con una sola BL por hijo mutante y un único `HEAD` base compartido al arranque.
  3. La declaración de una sesión como `parallel-ready` queda reservada al padre neutral y exige, como mínimo: repo limpio al inicio salvo `workspace_only_dirty`, `BL-061` y `BL-072` cerradas, `write_set` explícito y sin solape material por BL, surfaces seriales fuera de los `write_set`, y ruta independiente `gates -> auditor -> closeout` por BL antes de cualquier integración.
  4. La integración final sigue siendo siempre serial y exclusiva del padre neutral. Ningún hijo integra ramas, muta otra `worktree` ni cierra varias BL a la vez.
  5. Si aparece dirty inesperado, conflicto estructural, escape de `write_set` o rojo en gates/auditoría/closeout, la BL afectada queda fuera de integración y el flujo vuelve a reconciliación aislada; el éxito de una BL no fuerza la integración de las demás.
- **Razón:** El problema real no era que varias BL no pudieran progresar en paralelo, sino que faltaba un contrato explícito que preservara aislamiento, trazabilidad y cierre limpio. Canonizar `parallel-ready` como elegibilidad controlada permite abrir pilotos reales sin degradar la disciplina de repo limpio, sin reabrir la falsa idea de que paralelizar implica compartir árbol o mezclar cierres, y sin desplazar al padre neutral de su función de integración y control.
- **Impacto:** `BL-072` puede cerrarse como tarea de governance-only; `BL-073` queda desbloqueada documentalmente, pero no se convierte en prioridad obligatoria ni en permiso general para paralelizar. Cualquier piloto futuro debe seguir exactamente el proceso definido en `docs/project/ROLES.md` y podrá abortarse sin contaminar `main`.

## DEC-030 — Packet C / Hypothesis Basis Hardening
- **Fecha:** 2026-03-14
- **Contexto:** El runtime ya validaba consistencia interna, diff limpio, gates verdes y coherencia entre evidencia y outcome, pero no exigía demostrar que la hipótesis inicial de una BL `investigation` o `expansion` seguía siendo verdadera antes de empaquetarla. El patrón quedó expuesto por `BL-088`: TEP ya tenía acquire funcional por el carril validado (`IR crawler` + descarga HTTP desde `tp.com`) y mantenía `PASS 100.0% (109/109)`, pero el sistema entero aceptó una investigación sobre un gap que no existía como si fuera una premisa sana. El mismo olor estratégico ya había aparecido en decisiones previas como `DEC-020`, `DEC-022` y `DEC-023`: el ciclo era capaz de ejecutar trabajo internamente coherente sobre una premisa defectuosa.
- **Decisión:**
  1. **Ownership del pre-gate en el `director`.** El `orchestrator` mantiene su neutralidad y no ejecuta el hypothesis check. Cuando empaqueta una BL `investigation` o `expansion`, el `director` debe producir un `hypothesis_basis` que justifique que el gap declarado existe antes de mandarla a ejecución.
  2. **Bifurcación por `Work kind`.**
     - `investigation`: el `hypothesis_basis` se arma desde el `case.json` del anchor, `filings_manifest.json` cuando exista y sea relevante para la finding, el reporte `eval` del scout pass vigente y el item correspondiente en `docs/project/OPPORTUNITIES.md`. La pregunta a responder es si el ticker ancla sigue teniendo el gap factual declarado hoy.
     - `expansion`: el `hypothesis_basis` se arma desde el item curado en `docs/project/OPPORTUNITIES.md` y los criterios vigentes de elegibilidad de `expansion_candidate` en `docs/project/ROLES.md`. No exige `case.json` ni `eval` de un anchor inexistente. La pregunta a responder es si el candidato cumple esos criterios y si `OPPORTUNITIES.md` sostiene que el onboarding es viable.
  3. **Salida explícita del pre-gate.** El `director` debe clasificar el chequeo previo como `gap_confirmed: true | false | inconclusive`. Solo `true` permite empaquetar una BL. `false` e `inconclusive` abortan el packaging antes de tocar `docs/project/BACKLOG.md`.
  4. **Persistencia mínima y parseable en backlog.** Toda BL `investigation` o `expansion` que sí llegue a `docs/project/BACKLOG.md` debe persistir:
     - `Hypothesis basis subject`
     - `Hypothesis basis gap`
     - `Hypothesis basis evidence`
     - `Hypothesis basis confirmed`
     - `Hypothesis basis snapshot`
     `Hypothesis basis evidence` solo puede citar surfaces versionadas o valores reproducibles; nunca rutas `/tmp` ni artefactos efímeros. `Hypothesis basis snapshot` usa `HEAD + eval_signature` en `investigation` y `HEAD + n/a` en `expansion`. La forma rica del packet puede incluir `scout_context_signature`, pero esa firma no entra en la cola ejecutable canónica por compatibilidad con `DEC-027`.
  5. **Camino negativo con anti-regresión.** Si `gap_confirmed` sale `false` o `inconclusive`, no se abre BL. El `director` debe reconciliar `docs/project/OPPORTUNITIES.md`: actualizar `Live evidence` y/o `Unknowns remaining` con la nota factual que explica por qué el gap no quedó confirmado, mantener intacta `Disposition`, y refrescar `Last reviewed`. Un caso `inconclusive` no se reabre por timeout; solo por evidencia nueva o por override explícito de Elsian.
  6. **Challenge del `auditor` como red de seguridad.** El `auditor` pasa a leer los campos persistidos de `hypothesis_basis` desde `docs/project/BACKLOG.md` como superficie canónica. Debe reportar severidad alta cuando el gap no existía y la BL pierde sentido desde el inicio, y severidad media cuando el framing era mejorable pero la BL todavía produjo evidencia útil.
  7. **Scope.** El hardening es obligatorio para `investigation` y `expansion`. Para `technical`, queda como herramienta opcional del `director` cuando el motivo no es trivialmente observable desde el diff o el estado factual.
- **Razón:** El sistema necesitaba distinguir entre “ejecución internamente correcta” y “trabajo bien planteado”. Sin este pre-gate, una ola puede cerrar en verde aunque persiga un gap inexistente, desperdiciando presupuesto de batch y dando apariencia de rigor a una premisa falsa. Poner la carga de prueba en el `director`, persistir una versión mínima y auditable del basis en la cola canónica, y dar al `auditor` un challenge explícito endurece la calidad de la hipótesis sin convertir al `orchestrator` en juez técnico ni romper el modelo factual de `DEC-027`.
- **Impacto:** Las BL `investigation` y `expansion` dejan de poder nacer solo por inercia del scout o por framing heredado en `OPPORTUNITIES.md`. El coste operativo sube ligeramente en packaging, pero a cambio bajan los falsos gaps packageados y mejora la trazabilidad de por qué una investigación existía de verdad cuando entró en cola.

## DEC-031 — Eliminación de ANNUAL_ONLY como period_scope válido
- **Fecha:** 2026-03-15
- **Contexto:** Los agentes usan sistemáticamente `period_scope=ANNUAL_ONLY` como vía de escape para cerrar tickers con solo 2 períodos anuales cuando el corpus contiene filings quarterly completos (10-Qs descargados y disponibles). El patrón ha sido recurrente: BL-092 cerró DNOW, FRPH y HBB con eval 20/20 (2 anuales × 10 campos) cuando sus corpus SEC tienen 10-Qs que soportan 7-8 períodos. El evaluador (`evaluator.py`) solo comprueba lo que hay en `expected.json` e ignora `period_scope`, lo que permite que el agente declare `ANNUAL_ONLY`, cure solo anuales, pase eval verde y declare victoria sin tocar los quarterly.
- **Decisión:**
  1. **`ANNUAL_ONLY` deja de ser un valor válido de `period_scope`.** El único valor aceptado es `FULL`. `validate_contracts.py` rechaza cualquier `case.json` o `expected.json` que contenga `period_scope: "ANNUAL_ONLY"`.
  2. **`FULL` significa "todos los períodos que el corpus soporta están curados".** Para un ticker SEC con 10-Ks y 10-Qs, FULL exige que `expected.json` incluya tanto anuales como quarterly. Para un ticker ASX donde el regulador no publica quarterly, FULL con solo anuales en `expected.json` es correcto porque el corpus no contiene quarterly.
  3. **Enforcement cruzado en `validate_contracts.py`.** Cuando `filings_manifest.json` del mismo case dir contiene filings con `filing_kind=quarterly`, `expected.json` DEBE incluir al menos un período quarterly (key que matchee patrón `Q\d-\d{4}` o `H\d-\d{4}`). Si no los tiene, la validación falla con mensaje explícito.
  4. **Defaults actualizados.** Todos los defaults de `period_scope` en el código (`case.py`, `scaffold.py`, `discover.py`, `cli.py`, `coverage_audit.py`, `truth_pack.py`) pasan de `"ANNUAL_ONLY"` a `"FULL"`.
  5. **Tickers existentes.** KAR y JBH (ASX) pasan a clasificarse como `FULL` — sus `expected.json` solo contienen anuales, lo cual es correcto porque su corpus no tiene quarterly. SOM pasa a `FULL` como excepción reafirmada. Los tickers SEC que quedaron con solo anuales curados necesitan una ola de curación quarterly para cumplir el nuevo contrato.
  6. **Resultados terminales.** `onboarded_to_annual_only` deja de ser un outcome válido en `ROLES.md`. Las BL `expansion` pueden cerrar como `onboarded_full`, `technical_followup_opened` o `discarded_candidate`.
- **Razón:** `ANNUAL_ONLY` se diseñó para mercados sin quarterly, pero los agentes lo usan como escape para evitar el trabajo de curación quarterly en mercados donde sí existe. Eliminarlo como opción fuerza a los agentes a curar todo lo que el corpus soporta o a documentar explícitamente por qué no pueden (BLOCKED). El enforcement cruzado en `validate_contracts.py` cierra la segunda vía de escape: poner `period_scope=FULL` pero seguir curando solo anuales.
- **Impacto:** Los agentes ya no pueden cerrar un ticker SEC con eval 20/20 y declarar victoria. Si el corpus tiene quarterly, `expected.json` debe incluirlos o la validación falla. Los tickers ASX no se ven afectados funcionalmente — solo cambia la etiqueta de `ANNUAL_ONLY justificado` a `FULL`.
