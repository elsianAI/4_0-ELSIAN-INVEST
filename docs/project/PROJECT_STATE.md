...
`BL-005`, `BL-062`, `BL-063`, `BL-065`, `BL-066`, `BL-067`, `BL-069`, `BL-070`, `BL-071` y `BL-072` ya están cerradas y archivadas en `docs/project/BACKLOG_DONE.md`; ninguna sigue siendo una prioridad shared-core viva. En el backlog activo, la siguiente prioridad operativa es `BL-064`, y `BL-073` solo puede ejecutarse si el packet concreto pasa el checklist `parallel-ready` fijado en `docs/project/ROLES.md` y `DEC-029`. En particular, `BL-071` debe leerse como cierre de un slice estrecho de scaffolding (`scaffold-task` / `scaffold-case`) y no como absorción implícita del T15 amplio de plantillas y DX.
