...
- **Descripción:** Se cierra BL-093 como ola targeted filing-backed sobre cinco emisores SEC directos con corpus local ya presente. El cierre histórico absorbido en esta BL dejó `JELD`, `KELYA`, `MATW`, `NVRI` y `PHIN` canonizados solo con FY anuales vigentes del `extraction_result.json` local (`FY2024`, `FY2025`) y sin tocar shared-core ni redescargar filings. Ese cierre sigue siendo factual como snapshot técnico de 2026-03-15, pero deja de ser criterio suficiente tras `DEC-031`: la deuda quarterly requerida por el nuevo contrato se reabre aparte en `BL-096`, no dentro de esta BL ya archivada.
- **Criterio de aceptación:** ✓ Snapshot histórico absorbido: `cases/JELD/case.json` / `expected.json` PASS y `python3 -m elsian eval JELD` → PASS 100.0% (20/20). ✓ `cases/KELYA/case.json` / `expected.json` PASS y `python3 -m elsian eval KELYA` → PASS 100.0% (20/20). ✓ `cases/MATW/case.json` / `expected.json` PASS y `python3 -m elsian eval MATW` → PASS 100.0% (20/20). ✓ `cases/NVRI/case.json` / `expected.json` PASS y `python3 -m elsian eval NVRI` → PASS 100.0% (20/20). ✓ `cases/PHIN/case.json` / `expected.json` PASS y `python3 -m elsian eval PHIN` → PASS 100.0% (20/20). ✓ `BL-093` sale de `docs/project/BACKLOG.md`. ✓ Follow-up contractual posterior: `BL-096` absorbe la recuración quarterly exigida por `DEC-031`.
