...
SCOUT_SKILL_PATH = CODEX_HOME / "skills" / "elsian-capacity-scout" / "SKILL.md"
ORCHESTRATOR_SKILL_PATH = CODEX_HOME / "skills" / "elsian-orchestrator" / "SKILL.md"
KICKOFF_SKILL_PATH = CODEX_HOME / "skills" / "elsian-kickoff" / "SKILL.md"
DIRECTOR_SKILL_PATH = CODEX_HOME / "skills" / "elsian-director" / "SKILL.md"
...
def test_capacity_scout_mirrors_share_packet_b_contract() -> None:
    ...
    for path in (ROLE_PATH, SCOUT_AGENT_PATH, *_existing(SCOUT_SKILL_PATH)):
        text = _read(path)
...
def test_orchestrator_mirrors_reference_contract_driven_empty_backlog_routing() -> None:
    ...
    for path in (ORCHESTRATOR_AGENT_PATH, *_existing(ORCHESTRATOR_SKILL_PATH)):
        text = _read(path)
...
def test_director_mirrors_reference_packet_b_batch_rules() -> None:
    ...
    for path in (DIRECTOR_AGENT_PATH, *_existing(DIRECTOR_SKILL_PATH)):
        text = _read(path)
        for snippet in required:
            assert snippet in text, f"{snippet!r} missing in {path}"
...
def test_kickoff_mirrors_keep_empty_backlog_resolution_non_terminal() -> None:
    ...
    for path in (KICKOFF_AGENT_PATH, *_existing(KICKOFF_SKILL_PATH)):
        text = _read(path)
        for snippet in required:
            assert snippet in text, f"{snippet!r} missing in {path}"
