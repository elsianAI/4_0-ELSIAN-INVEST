...
- If the checker reports `empty_backlog_discovery`, report that state explicitly but do not launch discovery yourself; that step belongs to `orchestrator`, and the kickoff output is not sufficient to choose between scout findings, governance-only reconciliation, baseline-only wave, or later execution on its own.
