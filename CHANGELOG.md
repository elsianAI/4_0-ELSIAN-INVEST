...
- Esta reconciliación queda acotada a backlog + changelog; `PROJECT_STATE.md` se difiere hasta el cierre factual posterior de la secuencia SEC.
...
- `cases/DCBO/case.json` se canoniza lo mínimo para el ancla `40-F`, pero la repro live deja el bloqueo reusable explícito: `python3 -m elsian acquire DCBO` sigue descargando solo `SRC_001_40-F_FY2025.htm`…`SRC_018_6-K_Q2-2025.txt`, el wrapper FY2025 referencia el Exhibit `99.2` `docebo-20251231.htm`, ese fichero no se materializa en `cases/DCBO/filings/`, y `python3 -m elsian run DCBO --skip-assemble` permanece en `0 fields / 0 periods`.
- **Files changed:** `cases/DCBO/case.json`, `cases/DNOW/case.json`, `cases/DNOW/expected.json`, `cases/FRPH/case.json`, `cases/FRPH/expected.json`, `cases/HBB/case.json`, `cases/HBB/expected.json`, `CHANGELOG.md`
- **Validation:** `python3 scripts/validate_contracts.py --schema case --path cases/DCBO/case.json` → PASS; `python3 scripts/validate_contracts.py --schema case --path cases/DNOW/case.json` → PASS; `python3 scripts/validate_contracts.py --schema expected --path cases/DNOW/expected.json` → PASS; `python3 scripts/validate_contracts.py --schema case --path cases/FRPH/case.json` → PASS; `python3 scripts/validate_contracts.py --schema expected --path cases/FRPH/expected.json` → PASS; `python3 scripts/validate_contracts.py --schema case --path cases/HBB/case.json` → PASS; `python3 scripts/validate_contracts.py --schema expected --path cases/HBB/expected.json` → PASS; `python3 -m elsian eval DNOW` → `PASS 100.0% (20/20)`; `python3 -m elsian eval FRPH` → `PASS 100.0% (20/20)`; `python3 -m elsian eval HBB` → `PASS 100.0% (20/20)`; `python3 -m elsian acquire DCBO` → `18 filings downloaded (sec_edgar), Coverage 100.0%`; `python3 -m elsian run DCBO --skip-assemble` → `0 fields extracted`; `python3 -m elsian eval DCBO` → `FAIL 0.0% (0/0)`.
