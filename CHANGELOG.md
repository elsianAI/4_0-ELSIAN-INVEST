...
- **What:** Added a narrow Provenance L3 builder without reopening the extraction pipeline. `elsian/assemble/source_map.py` now generates `SourceMap_v1` from `extraction_result.json`, and the new CLI command `elsian source-map {TICKER}` writes `source_map.json` or an explicit output path. The pilot resolves three concrete click-to-source modes from existing L2 provenance: iXBRL facts point back to the original `.htm` via char offsets and DOM `id` anchors when present, HTML table fields point to the exact `.clean.md` line containing the extracted row, and `vertical_bs` text cases point to the matching `.txt` line. The validated pilot is TZOO, which now resolves 851/851 extracted fields into traceable source targets without modifying extractor winners, merge logic, or TruthPack.
- **Files changed:** `.gitignore`, `elsian/assemble/source_map.py`, `elsian/cli.py`, `tests/unit/test_source_map.py`, `tests/integration/test_source_map.py`, `docs/project/MODULE_1_ENGINEER_CONTEXT.md`, `docs/project/BACKLOG.md`, `docs/project/BACKLOG_DONE.md`, `docs/project/PROJECT_STATE.md`, `ROADMAP.md`, `CHANGELOG.md`
- **Tests:** `python3 -m pytest -q tests/unit/test_source_map.py tests/integration/test_source_map.py` → 5 passed. `python3 -m ruff check elsian/assemble/source_map.py elsian/cli.py tests/unit/test_source_map.py tests/integration/test_source_map.py` → clean. `python3 -m elsian eval TZOO` → PASS 100.0% (300/300). `python3 -m elsian source-map TZOO --output <tmp>/tzoo_source_map.json` → `SourceMap_v1`, 851/851 fields resolved (`clean_md_table`: 376, `html_ixbrl`: 458, `text_label`: 17).
