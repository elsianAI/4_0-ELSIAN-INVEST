...
### [4.0] BL-059 — Versioned contracts and centralized artifact validation
- **What:** Added versioned schemas under `schemas/v1/` for the active Module 1 artifacts (`case.json`, `expected.json`, `filings_manifest.json`, `extraction_result.json`, `truth_pack.json`, `source_map.json`, `task_manifest`). Added `scripts/validate_contracts.py` to validate only git-tracked artifacts against those contracts and to assert prompt/model alignment, keeping local drafts outside the gate. Expanded `elsian/models/case.py` so `CaseConfig` round-trips the real `case.json` contract instead of a misleading subset. Updated `.github/prompts/curate-expected.prompt.md` to remove legacy `deterministic/...` paths, switch from 22 to 29 canonical fields, and align the restatement example with `evidence_filing`.
- **Files changed:** `.github/prompts/curate-expected.prompt.md`, `elsian/models/case.py`, `schemas/v1/*`, `scripts/validate_contracts.py`, `tests/contracts/test_validate_contracts.py`, `tests/unit/test_models.py`
- **Tests:** `python3 scripts/validate_contracts.py --all` → PASS. `python3 -m pytest -q tests/contracts/test_validate_contracts.py tests/unit/test_models.py` → 12 passed. `python3 -m ruff check elsian/models/case.py scripts/validate_contracts.py tests/contracts/test_validate_contracts.py tests/unit/test_models.py` → clean. `python3 -m mypy elsian/models/case.py` → success. `git diff --check` on the BL-059 write set → clean.
